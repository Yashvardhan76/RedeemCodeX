...
class DeleteSubCommand(plugin: RedeemX) : JSubCommand {
    ...
    private fun deleteCode(code: String, placeHolder: CodePlaceHolder) {
        ...
        if (codeRepo.deleteCode(code)) config.sendMsg(JMessage.Code.Delete.SUCCESS, placeHolder)
        else config.sendMsg(JMessage.Code.Delete.NOT_FOUND, placeHolder)
    }

    private fun deleteCodes(codes: List<String>, placeHolder: CodePlaceHolder) {
        placeHolder.code = codes.joinToString(" ")
        if (codeRepo.deleteCodes(codes)) config.sendMsg(JMessage.Code.Delete.SUCCESS_CODES, placeHolder)
        else config.sendMsg(JMessage.Code.Delete.NOT_FOUND_ALL, placeHolder)
    }

    private fun deleteAllCodes(placeHolder: CodePlaceHolder) {
        if (codeRepo.getCachedCode().isEmpty()) {
            config.sendMsg(JMessage.Code.Delete.NOT_FOUND_ALL, placeHolder)
            return
        }
        codeRepo.deleteAllCodes()
        config.sendMsg(JMessage.Code.Delete.SUCCESS_ALL, placeHolder)
    }

    private fun deleteTemplate(template: String, placeHolder: CodePlaceHolder): Boolean {
        placeHolder.template = template
        if (template == "DEFAULT") {
            config.sendMsg(JMessage.Template.Delete.FAILED_DEFAULT, placeHolder)
            return false
        }
        if (config.getTemplate(template) == null) {
            config.sendMsg(JMessage.Template.Delete.NOT_FOUND, placeHolder)
            return false
        }
        config.deleteTemplate(template)
        config.sendMsg(JMessage.Template.Delete.SUCCESS, placeHolder)
        return true
    }

    private fun deleteAllTemplates(placeHolder: CodePlaceHolder): Boolean {
        config.deleteAllTemplates()
        config.sendMsg(JMessage.Template.Delete.SUCCESS_ALL, placeHolder)
        return true
    }
        ...
        when (args[1].lowercase()) {
            JTab.Type.CODE -> {
                ...
                if (codes.first() == JTab.Delete.All.value) {
                    if (args.size < 4 || args[3] != JTab.Delete.Confirm.value) return config.sendMsg(
                        JMessage.Code.Delete.CONFIRMATION_NEEDED, placeHolder
                    ) != Unit
            ...
            JTab.Type.TEMPLATE -> {
                ...
                if (templates.first() == JTab.Delete.All.value) {
                    if (args.size < 4 || args[3] != JTab.Delete.Confirm.value) return config.sendMsg(
                        JMessage.Template.Delete.CONFIRMATION_NEEDED, CodePlaceHolder(sender)
                    ) != Unit
                    deleteAllTemplates(CodePlaceHolder(sender))
                    jList = mutableListOf(JTab.Delete.All.value)
                    return true
                } else if (!args.getOrNull(3).isNullOrBlank()) {
